...
from components.subcomponents.matplotlib_presets import TBPARSE_PRESET_NAME_MAP, PLOT_PRESET_NAME_MAP, ALL_PRESET_NAME_MAP, get_plot_preset
import io
...
def matplotlib_figure(df_param, tbparse_reader_param):
    ...
    if col3.button('All charts'):
        state[PLOT_PRESETS_STATE] = list({**PLOT_PRESET_NAME_MAP, **TBPARSE_PRESET_NAME_MAP}.keys())
        state[CODE_STATE] = get_plot_preset(state.plot_presets)
    selected_presets = col2.multiselect('Presets', options=ALL_PRESET_NAME_MAP.keys(),
        key=PLOT_PRESETS_STATE, on_change=update_code_state_callback)
    with col2:
        matplotlib_code = st_ace(value=state.get(CODE_STATE, ''), language='python', height='400px', placeholder=CODE_PLACEHOLDER_TEXT)

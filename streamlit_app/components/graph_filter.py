...
def win_rate_by_adv_steps_graph_filter(df):
    ...
    win_rate_df["adv = white"] = (
        df[df.adv_color == "w"].groupby("adv_steps").mean()["adv_win"]
    )
    ...
    lower_step, upper_step = st.slider(
        label="",
        min_value=min_step,
        max_value=max_step,
        value=(min_step, max_step),
        key=TRAINING_STEPS_SLIDER_STATE,
        step=(max_step - min_step) // 100,
    )

    px_fig.add_vrect(
        x0=lower_step, x1=upper_step, line_width=0, fillcolor="red", opacity=0.2
    )

...
def data_loader():
    ...
    state[TBPARSE_EVENT_TYPES_STATE] = state.get(TBPARSE_EVENT_TYPES_STATE) or TBPARSE_EVENT_TYPES[0:1]
    event_types = col2.multiselect('Tbparse event types', TBPARSE_EVENT_TYPES, key=TBPARSE_EVENT_TYPES_STATE)
    ...
    if df_unfiltered_len > 0:
        st.markdown(f"Loaded {df_unfiltered_len} games from: `{state[DATA_LOAD_ARGS_STATE]['data_source']}`")

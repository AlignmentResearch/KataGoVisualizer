...
def st_directory_picker(ui_parent, key=None):
    ...
    with col3:
        ...
        selected_dir = ui_parent.selectbox("Subdirectories", sorted(subdirectroies))
        st.session_state.new_dir = selected_dir

    with col2:
        if read_dir in session_path.parents:
            ui_parent.markdown("#")
            ui_parent.button("⬅️", on_click=change_dir, args=(key, session_path.parent))

    with col4:
        if 'new_dir' in st.session_state and st.session_state['new_dir'] is not None:
            ui_parent.markdown("#")
            ui_parent.button("➡️", on_click=change_dir, args=(key, session_path / st.session_state.new_dir))

    with col1:
        selected_path = ui_parent.text_input("Selected directory:", key=key)

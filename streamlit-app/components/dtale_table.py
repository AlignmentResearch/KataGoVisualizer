...
DTALE_DEFAULT_SETTINGS_ADJUST = {'allow_cell_edits': False, 'hide_shutdown': True, "columnFilters":{"board_size":{"value":[19], "operand":"=", "query":"`board_size` == 19"}}}
...
def dtale_table_and_go_board(df, lower_step, upper_step):
    ...
    state[DTALE_SETTINGS_STATE] = global_state.get_settings(dtale_data_id)
    state[DTALE_SETTINGS_STATE]['columnFilters']['adv_steps'] = {'operand': '[]', 'min': lower_step, 'max': upper_step, 'query': f'`adv_steps` >= {lower_step} and `adv_steps` <= {upper_step}'}
    global_state.set_settings(dtale_data_id, state[DTALE_SETTINGS_STATE])
    ...
    if st.button(f'View selected game'):
    ...
    if state.get(SGF_ROW_STATE): # Rows are 1-indexed
        ...
        if final_query:
            df = run_query(handle_predefined(dtale_data_id), final_query, global_state.get_context_variables(dtale_data_id), ignore_empty=True)

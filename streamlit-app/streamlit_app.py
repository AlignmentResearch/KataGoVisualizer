...
def verify():
    state[PW_HASH_STATE] = hashlib.scrypt(str.encode(state[PW_STATE]), salt=b'salt', n=2048, r=8, p=1)

if state.get(PW_HASH_STATE) != HASH:
    password = st.text_input('Password', type='password', on_change=verify, key=PW_STATE)
